...
# Paleta (la misma de la plantilla) y formatos.
_AZUL_OSCURO = "FF1F3864"
_AZUL_MEDIO = "FF305496"
_AZUL_CLARO = "FFDCE6F1"
_GRIS_CLARO = "FFF2F2F2"
_VERDE_OK = "FFC6E0B4"
_DURAZNO = "FFFCE4D6"
_FMT_USD = '"US$ "#,##0'
_FMT_SOL = '"S/ "#,##0'
...
# A = margen; B:E = tablas; F = separador; G:J = gráficos.
_ANCHOS_DASHBOARD = {
    "A": 3, "B": 30, "C": 15, "D": 15, "E": 17,
    "F": 2, "G": 17, "H": 17, "I": 17, "J": 17,
}
_BORDE_FINO = Border(*(Side(style="thin", color="FFBFBFBF"),) * 4)
...
def _celda(ws, ref: str, valor=None, *, fill=None, color=None, size=11,
           bold=False, align="center", fmt=None, wrap=False, borde=False):
    """Escribe (y opcionalmente combina) una celda con estilo."""
    if ":" in ref:
        ws.merge_cells(ref)
        ref = ref.split(":")[0]
    cel = ws[ref]
    if valor is not None:
        cel.value = valor
    cel.font = Font(name="Arial", size=size, bold=bold, color=color or _AZUL_OSCURO)
...
def _monto_usd(f: dict, ret_cfg, tc: float) -> float:
    """Neto de una factura llevado a dólares (los soles se dividen por el T/C)."""
    neto = _neto(f, ret_cfg)
    if str(f.get("MONEDA", "")).strip().upper() == "SOL":
        return neto / tc if tc else 0.0
    return neto


def _vencimientos_por_semana(filas: list[dict], ret_cfg, tc: float) -> list[tuple]:
    """[(etiqueta, soles, dólares, total US$)] por semana de vencimiento."""
    sem: dict = {}
    for f in filas:
        d = _fecha(f.get("FEC. VCTO"))
        clave = d - timedelta(days=d.weekday()) if isinstance(d, date) else None
        sol, usd = sem.setdefault(clave, [0.0, 0.0])
        neto = _neto(f, ret_cfg)
        if str(f.get("MONEDA", "")).strip().upper() == "SOL":
            sem[clave][0] = sol + neto
        else:
            sem[clave][1] = usd + neto

    def _fila(etiqueta, sol, usd):
        total = (sol / tc if tc else 0) + usd
        return (etiqueta, round(sol, 2), round(usd, 2), round(total, 2))

    filas_out = []
    for clave in sorted(k for k in sem if k is not None):
        fin = clave + timedelta(days=6)
        filas_out.append(_fila(f"{clave:%d/%m} - {fin:%d/%m}", *sem[clave]))
    if None in sem:
        filas_out.append(_fila("Sin fecha", *sem[None]))
    return filas_out


def _top_proveedores(filas: list[dict], ret_cfg, tc: float) -> tuple:
    """(top N [(proveedor, monto US$)], monto del resto, cuántos son el resto)."""
    acc: dict = {}
    for f in filas:
        nombre = str(f.get("PROVEEDOR", "")).strip() or "(sin nombre)"
        acc[nombre] = acc.get(nombre, 0.0) + _monto_usd(f, ret_cfg, tc)
    orden = sorted(acc.items(), key=lambda x: -x[1])
    resto = orden[_TOP_N_PROVEEDORES:]
    top = [(n, round(v, 2)) for n, v in orden[:_TOP_N_PROVEEDORES]]
    return top, round(sum(v for _, v in resto), 2), len(resto)


def _kpis_desde_resumen(wb) -> list[tuple[str, str]]:
    """KPIs como fórmulas al Resumen (se mantienen vivos)."""
    if "Resumen" not in wb.sheetnames:
        return []
    res = wb["Resumen"]
    f_cons = _fila_etiqueta(res, "TOTAL CONSOLIDADO")
    f_banda = _fila_etiqueta(res, "ESTADO DE LIQUIDEZ")
    f_alertas = _fila_etiqueta(res, "V. ALERTAS")
    val = f_banda + 1 if f_banda else None
    kpis = []
    if f_cons:
        kpis.append(("TOTAL A PAGAR (US$)", f"=+Resumen!D{f_cons}"))
    if val:
        kpis.append(("SALDO DISPONIBLE (US$)", f"=+Resumen!C{val}"))
        kpis.append(("SALDO PROYECTADO (US$)", f"=+Resumen!D{val}"))
    if f_alertas:
        kpis.append((
            "A VENDER / TRANSFERIR (US$)",
            f"=SUM(Resumen!D{f_alertas + 2}:D{f_alertas + 4})",
        ))
    return kpis


def _tabla_dashboard(ws, fila: int, titulo: str, encabezados: list[str],
                     datos: list[tuple], formatos: list[str], total: tuple | None):
    """Escribe una sección con título, encabezados, datos y fila TOTAL.
    Devuelve (fila_encabezados, fila_primer_dato, fila_ultimo_dato, fila_siguiente)."""
    ncols = len(encabezados)
    ultima_col = get_column_letter(1 + ncols)
    _celda(ws, f"B{fila}:{ultima_col}{fila}", titulo, fill=_AZUL_OSCURO,
           color="FFFFFFFF", size=11, bold=True, align="left")
    ws.row_dimensions[fila].height = 20

    f_hdr = fila + 1
    for i, texto in enumerate(encabezados):
        _celda(ws, f"{get_column_letter(2 + i)}{f_hdr}", texto, fill=_GRIS_CLARO,
               size=9, bold=True, borde=True)

    r = f_hdr + 1
    for registro in datos:
        for i, valor in enumerate(registro):
            _celda(ws, f"{get_column_letter(2 + i)}{r}", valor, size=10,
                   align="left" if i == 0 else "right", fmt=formatos[i], borde=True)
        r += 1
    f_fin = r - 1

    if total is not None:
        for i, valor in enumerate(total):
            _celda(ws, f"{get_column_letter(2 + i)}{r}", valor, fill=_AZUL_CLARO,
                   size=10, bold=True, align="left" if i == 0 else "right",
                   fmt=formatos[i], borde=True)
        r += 1
    return f_hdr, f_hdr + 1, f_fin, r


def _grafico(ws, tipo: str, titulo: str, col_datos: int, f_hdr: int, f_fin: int,
             ancla: str):
    """Gráfico de barras sobre una columna de la tabla (categorías en B)."""
    ch = BarChart()
    ch.type = tipo
    ch.title = titulo
    ch.legend = None
    ch.y_axis.majorGridlines = None
    ch.height, ch.width = 7.5, 11.5
    ch.add_data(
        Reference(ws, min_col=col_datos, min_row=f_hdr, max_row=f_fin),
        titles_from_data=True,
    )
    ch.set_categories(Reference(ws, min_col=2, min_row=f_hdr + 1, max_row=f_fin))
    ws.add_chart(ch, ancla)


def _construir_dashboard_sheet(wb, filas, ret_cfg, tc, fecha_inicio, fecha_final):
    """Crea la hoja 'Dashboard' (primera) con KPIs, semáforo, vencimientos por
    semana y top de proveedores. Debe correr DESPUÉS de armar el Resumen."""
    if "Dashboard" in wb.sheetnames:
        del wb["Dashboard"]
    ws = wb.create_sheet("Dashboard", 0)
    for col, ancho in _ANCHOS_DASHBOARD.items():
        ws.column_dimensions[col].width = ancho
    ...
    if fecha_inicio or fecha_final:
        rango = f" {fecha_inicio or ''}{' a ' + fecha_final if fecha_final else ''}".rstrip()
    _celda(ws, "B2:J2", "DASHBOARD EJECUTIVO", fill=_AZUL_OSCURO,
           color="FFFFFFFF", size=16, bold=True)
    ws.row_dimensions[2].height = 30
    _celda(ws, "B3:J3", f"PAGOS PROVEEDORES{rango}", size=10, color="FF595959")
    ws.row_dimensions[3].height = 16

    # --- KPIs (fórmulas vivas al Resumen) ---
    bloques = ["B{r}:C{r}", "D{r}:E{r}", "G{r}:H{r}", "I{r}:J{r}"]
    for (etiqueta, formula), bloque in zip(_kpis_desde_resumen(wb), bloques):
        _celda(ws, bloque.format(r=5), etiqueta, fill=_AZUL_MEDIO,
               color="FFFFFFFF", size=9, bold=True)
        _celda(ws, bloque.format(r=6), formula, fill=_AZUL_CLARO, size=16,
               bold=True, fmt=_FMT_USD)
    ws.row_dimensions[5].height = 16
    ws.row_dimensions[6].height = 30

    # --- Semáforo de acción (texto del Resumen, con color según el estado) ---
    res_banda = _fila_etiqueta(wb["Resumen"], "ESTADO DE LIQUIDEZ") if "Resumen" in wb.sheetnames else None
    if res_banda:
        _celda(ws, "B8:J8", f"=+Resumen!A{res_banda + 1}", fill=_DURAZNO,
               size=14, bold=True)
        ws.row_dimensions[8].height = 26
        ws.conditional_formatting.add(
            "B8",
            CellIsRule(operator="equal", formula=['"LIQUIDEZ SUFICIENTE"'],
                       fill=PatternFill("solid", fgColor=_VERDE_OK)),
        )

    # --- Vencimientos por semana ---
    venc = _vencimientos_por_semana(filas, ret_cfg, tc)
    f_hdr, _, f_fin, sig = _tabla_dashboard(
        ws, 10, "VENCIMIENTOS POR SEMANA",
        ["Semana de vencimiento", "Soles (S/)", "Dólares (US$)", "Total (US$)"],
        venc, [None, _FMT_SOL, _FMT_USD, _FMT_USD],
        ("TOTAL", sum(v[1] for v in venc), sum(v[2] for v in venc),
         sum(v[3] for v in venc)) if venc else None,
    )
    if venc:
        ws.conditional_formatting.add(
            f"E{f_hdr + 1}:E{f_fin}",
            DataBarRule(start_type="num", start_value=0, end_type="max",
                        color=_AZUL_MEDIO[2:], showValue=True),
        )
        _grafico(ws, "col", "Vencimientos por semana (US$)", 5, f_hdr, f_fin, "G10")

    # --- Top proveedores ---
    top, resto_monto, resto_n = _top_proveedores(filas, ret_cfg, tc)
    total_usd = round(sum(v for _, v in top) + resto_monto, 2)
    datos = [(n, v, (v / total_usd if total_usd else 0)) for n, v in top]
    if resto_n:
        datos.append((f"Otros ({resto_n} proveedores)", resto_monto,
                      resto_monto / total_usd if total_usd else 0))
    fila_top = max(sig + 1, 26)
    f_hdr2, _, f_fin2, _ = _tabla_dashboard(
        ws, fila_top, f"TOP {_TOP_N_PROVEEDORES} PROVEEDORES",
        ["Proveedor", "Monto (US$)", "% del total"],
        datos, [None, _FMT_USD, _FMT_PCT],
        ("TOTAL", total_usd, 1 if total_usd else 0) if datos else None,
    )
    if datos:
        ws.conditional_formatting.add(
            f"C{f_hdr2 + 1}:C{f_fin2}",
            DataBarRule(start_type="num", start_value=0, end_type="max",
                        color=_AZUL_MEDIO[2:], showValue=True),
        )
        _grafico(ws, "bar", "Top proveedores (US$)", 3, f_hdr2, f_fin2,
                 f"G{fila_top}")
...
def construir_detalle(
    ...
    # Dashboard (primera hoja): va al final porque sus KPIs referencian filas
    # del Resumen ya reacomodado. Analiza las facturas que sí van al informe.
    filas_informe = [f for fs in grupos.values() for f in fs]
    filas_informe += [f for fs in grupos_agentes.values() for f in fs]
    _construir_dashboard_sheet(
        wb, filas_informe, ret_cfg, tc, fecha_inicio, fecha_final
    )
